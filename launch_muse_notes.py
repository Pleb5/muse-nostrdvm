...
from nostr_dvm.tasks.muse_notes import build_muse_notes
...
async def configure_and_start_DVM(openai_client: openai.AsyncOpenAI, env_path: Path):
    try:
        ...
        nip89info = {
                "name": name,
                "picture": image,
                "about": "Inspirational Notes for Freelancers",
        ...
        options = {
            "max_results": 200,
            "db_name": "db/muse-notes", 
            "db_since": 3600 * 24 * 21, # last 21 days
            "max_db_size" : 1024,
            # "personalized": False
        }

        dvm = await build_muse_notes(
            name,
            openai_client,
            dvm_config,
            nip89config,
